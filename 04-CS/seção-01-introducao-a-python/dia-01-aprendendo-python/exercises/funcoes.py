# Por meio do vídeo, notamos que funções são definidas através da palavra reservada def, 
# seguida por um nome e os parâmetros entre parênteses. Como todo bloco de código em Python, 
# o caractere : define o início do bloco, e a indentação define seu fim.
...
# Anota aí ✏️: Parâmetros posicionais variádicos são acessados como uma tupla no interior de uma 
# função, e parâmetros nomeados variádicos como um dicionário.

def concat(*strings):
    # Equivalente a um ", ".join(strings), que concatena os elementos de um iterável em uma string 
...
# As variáveis definidas dentro das funções tem escopo local. Porém, quando uma função não encontra um 
# nome no escopo local, ela irá procurar no espaço de nomes global.
